...
CREATE OR REPLACE FUNCTION tripleIdentity(id integer, OUT identity bytea) RETURNS bytea AS $tripleIdentity$
...
CREATE TABLE subgraph_replicas (
       ...
       UNIQUE (s, subgraph_identity, data_or_metadata_identity, replica), -- the same subject can participat in multiple replicas of the same subgraph
       UNIQUE (s_blank, subgraph_identity, data_or_metadata_identity, replica) -- a single free subgraph may appear multiple times
);
