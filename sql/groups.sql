...
CREATE FUNCTION email_complete() RETURNS trigger AS $email_complete$
       BEGIN
           IF NEW.email_validated AND
           NOT EXISTS (SELECT ue.email_validated FROM user_emails AS ue WHERE ue.user_id = NEW.user_id AND ue.email != NEW.email AND ue.email_validated = TRUE) AND -- OLD? not u_e?
