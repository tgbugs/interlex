...
CREATE FUNCTION orcid_complete() RETURNS trigger AS $orcid_complete$
       BEGIN
           ...
           IF EXISTS (SELECT * FROM user_emails as ue WHERE ue.user_id = NEW.id AND ue.email_validated IS NOT NULL LIMIT 1) AND
           OLD.orcid_validated IS NULL AND NEW.orcid_validated IS NOT NULL THEN -- only try this the first time
...
CREATE FUNCTION email_complete() RETURNS trigger AS $email_complete$
       BEGIN
           ...
           EXISTS (SELECT * FROM users as u WHERE u.id = NEW.user_id AND u.orcid_validated IS NOT NULL) THEN
