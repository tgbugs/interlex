...
CREATE UNIQUE INDEX ON perspectives (group_id) WHERE default_group_perspective;

CREATE FUNCTION def_pers_name_check() RETURNS trigger AS $$
BEGIN
    IF (NEW.default_group_perspective AND NOT EXISTS (SELECT * FROM groups AS g WHERE g.id = NEW.group_id AND g.groupname = NEW.name)) THEN
       RAISE EXCEPTION 'default perspective name must match groupname';
    ELSIF (NOT NEW.default_group_perspective AND EXISTS (SELECT * FROM groups AS g WHERE g.id = NEW.group_id AND g.groupname = NEW.name)) THEN
       RAISE EXCEPTION 'non default perspective name must not match groupname';
    END IF;
    RETURN NEW;
END;
$$ language plpgsql;

CREATE TRIGGER def_pers_name_check BEFORE INSERT ON perspectives FOR EACH ROW EXECUTE PROCEDURE def_pers_name_check();
...
CREATE TABLE groups(
       ...
       CHECK (groupname !~* '(tmp|ilx|cde|fde|pde)_.*'),
...
CREATE FUNCTION pop_group_pers() RETURNS trigger AS $$
BEGIN
    INSERT INTO perspectives (group_id, name, default_group_perspective) VALUES (NEW.id, NEW.groupname, TRUE);
    RETURN NULL;
END;
$$ language plpgsql;

CREATE TRIGGER pop_group_pers AFTER INSERT ON groups FOR EACH ROW EXECUTE PROCEDURE pop_group_pers();
...
CREATE TABLE display_configuration(
       -- TODO see whether we want to let users configure this at all ... we might
       -- TODO see if there is a use case for keying on perspectives
       group_id integer PRIMARY KEY references groups (id),
       config jsonb NOT NULL
);

CREATE TABLE display_configuration_history(
       id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
       group_id integer references groups (id),
       config jsonb NOT NULL,
