...
CREATE TABLE pull_requests(
       ...
       acting_user integer NOT NULL references users (id),
...
CREATE UNIQUE INDEX ON pull_requests (subject, from_perspective, to_perspective) WHERE status != 'merged';

CREATE TABLE pull_logs(
       id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
       pull_id integer,
       acting_user integer references users (id), -- FIXME can't quite do this with a trigger? would need a last_touch_user or something?
       -- this is the datetime the status was set, should include first record as well so we know what the original status was
       status pull_status_type NOT NULL,
       datetime timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
       UNIQUE (pull_id, acting_user, status, datetime)
);

CREATE OR REPLACE FUNCTION pull_log() RETURNS trigger as $$
BEGIN
INSERT INTO pull_logs (pull_id, acting_user, status) VALUES (NEW.id, NEW.acting_user, NEW.status);
RETURN NEW;
END;
$$ language plpgsql;

DROP TRIGGER IF EXISTS pull_log ON pull_requests;
CREATE TRIGGER pull_log AFTER INSERT OR UPDATE ON pull_requests
       FOR EACH ROW EXECUTE PROCEDURE pull_log();

CREATE OR REPLACE FUNCTION newPull(acting_groupname text, subject uri, from_group text, to_group text, from_pers_name text, to_pers_name text) RETURNS integer AS $$
DECLARE
uid_acting integer;
gid_from integer;
gid_to integer;
...
INSERT INTO pull_requests (subject, from_perspective, to_perspective, original_from_identity, original_to_identity, acting_user)
VALUES (newPull.subject, pers_from, pers_to, ofi, oti, uid_acting) RETURNING id INTO newid;
...
CREATE OR REPLACE FUNCTION closeUnclosePull(acting_groupname text, pull_id integer, reopen boolean DEFAULT FALSE) RETURNS VOID AS $$
DECLARE
uid_acting integer;
ok_role group_role;
to_group integer;
...
-- close/unclose only if
-- acting user is submitter or group.user_role < view ? or user has permissions for perspective-from ??? because we don't track submitter without the logs
-- status = pending / status = closed

SELECT up.user_role, pt.group_id, pf.group_id INTO ok_role, to_group, from_group
FROM pull_requests AS prs
JOIN perspectives AS pf ON pf.id = prs.from_perspective
JOIN perspectives AS pt ON pt.id = prs.to_perspective
LEFT OUTER JOIN user_permissions AS up
ON up.user_id = uid_acting AND up.user_role < 'view' AND (up.group_id = pf.group_id OR up.group_id = pt.group_id)
WHERE prs.id = pull_id AND (prs.status = 'pending' AND NOT reopen OR prs.status = 'closed' AND reopen);

IF to_group != uid_acting AND from_group != uid_acting AND ok_role IS NULL THEN
  RAISE EXCEPTION 'pull request was not (un)closed due to bad user permissions pull id is %', pull_id;
ELSIF to_group IS NOT NULL THEN
...
ELSE
  RAISE EXCEPTION 'pull request was not (un)closed for some reason pull id is %', pull_id;
END IF;

END;
$$ language plpgsql;

CREATE OR REPLACE FUNCTION lockUnlockPull(acting_groupname text, pull_id integer, unlock boolean DEFAULT FALSE) RETURNS VOID AS $$
DECLARE
uid_acting integer;
ok_role group_role;
to_group integer;
...
-- lock/unlock only if
-- acting user is group.user_role < view
-- status = pending / status = locked

SELECT up.user_role, pt.group_id INTO ok_role, to_group -- we use pt.id because pht might join empty if no perspective head is set
FROM pull_requests AS prs
JOIN perspectives AS pf ON pf.id = prs.from_perspective
JOIN perspectives AS pt ON pt.id = prs.to_perspective
LEFT OUTER JOIN user_permissions AS up ON up.user_id = uid_acting AND up.user_role < 'view' AND up.group_id = pt.group_id
WHERE prs.id = pull_id AND (prs.status = 'pending' AND NOT unlock OR prs.status = 'locked' AND unlock);

IF to_group != uid_acting AND ok_role IS NULL THEN
  RAISE EXCEPTION 'pull request was not (un)locked due to bad user permissions pull id is %', pull_id;
ELSIF to_group IS NOT NULL THEN

IF unlock THEN
UPDATE pull_requests
SET acting_user = uid_acting,
    status = 'pending' -- FIXME TODO consider whether we can lock closed pulls?
WHERE id = pull_id;
ELSE
UPDATE pull_requests
SET acting_user = uid_acting,
...
ELSE
  RAISE EXCEPTION 'pull request was not (un)locked for some reason pull id is %', pull_id;
END IF;
