...
=TEMP_DIR= must also be created and set before running any of the
scripts and should be cleaned up after everything is finished.
...
#+begin_src bash
pushd ${ALT_PATH} &&
# yeah teamptarget is because we have to sync to somewhere the ssh user can deploy and only then move alt to
# the runtime user
rsync --archive --verbose alt.zip ${INTERLEX_DEPLOY_USER}@${INTERLEX_SERVER}:${TEMP_DIR}/alt.zip || exit 20
popd || exit 3
#+end_src
...
#+name: &config-remote-root-commands-0
#+begin_src bash
...
sudo <<&config-remote-root-commands-0>>
pushd /var/lib/interlex || exit 22
<<&sudo-runtime-user>> <<&config-remote-runtime-user-commands-1>>
sudo <<&config-remote-root-commands-1>>
pushd run  || exit 10
<<&sudo-runtime-user>> <<&config-remote-runtime-user-commands-2>>
popd || exit 12
<<&sudo-runtime-user>> <<&config-remote-runtime-user-commands-3>>
if [ ! -s .mypass ]; then
...
<<&sudo-runtime-user>> <<&remote-command-runtime-user-commands-1>>
pushd run || exit 2
<<&sudo-runtime-user>> <<&remote-command-runtime-user-commands-2>>
popd || exit 4
popd || exit 5
<<&restart-ilx>>
#+end_src
