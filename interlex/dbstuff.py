...
class Stuff:
    ...
    def user_new(self, username, argon2_string, orcid, email):
...
'''
    ...
    def insertOrcidMetadata(self, orcid, name, token_type, token_scope, access_token, refresh_token, lifetime_seconds):
        args = dict(
            orcid=orcid,
            name=name,
        ...
        sql = '''
INSERT INTO orcid_metadata (orcid, name, token_type, token_scope, acccess_token, refresh_token, lifetime_seconds)
VALUES (:orcid, :name, :token_type, :token_scope, :acccess_token, :refresh_token, :lifetime_seconds)
'''
        return self.session_execute(sql, args)

    def getUserByOricd(self, orcid):
        args = dict(orcid=orcid)
        sql = 'SELECT * FROM users WHERE orcid = :orcid'
        return list(self.session_execute(sql, args))
